...
% Number of latent causes (ncop x initial # of hypotheses)
L     = par.L*par.ls.ncop;  
...
if isempty(agent)   
    ...
    if isfield(par,'initialV')
        ...
        agent.B = zeros(par.C,par.C,par.D,par.L);
        for d=1:par.D
            for l=1:L
...
% Update of agent    
    ...
    mu  =  reshape(sum(V.*c_vec,2), par.D,L,par.A); 
...
if par.t~=1 && isfield(par,'time')
    ...
    lsb(1:Lmax*par.L)  = lsb(1:Lmax*par.L)*prob_stay + (1-prob_stay)/(Lmax*par.L);
end
...
%--------------------------------------------------------------------------
function LL = LogLikelihood(win,mu,sigmasq,Lmax,L,ncop)
% Log-likelihood of each latent state

% Initialize
LL                         =  zeros(1,L);
LL((L/ncop)*(Lmax+1)+1:end)= -Inf;
...
tmp      = zeros(1,par.L*par.ls.ncop);
tmp(ix1) = ((1-par.ls.gamma)*lsb(ix1)+par.ls.gamma/(Lmax*par.L)).*exp(LL(ix1)-max(LL(ix1)));
...
if Lmax < par.ls.ncop    
    ...
    tmp(ix1) = ((1-par.ls.gamma)*lsb(ix1)+par.ls.gamma/(Lmax*par.L)).*exp(LL(ix1)-mx);
    ...
    if q >= par.ls.eta
        ...
        % Update beliefs
        tmp([ix1,ix2]) = ((1-par.ls.gamma)*lsb([ix1,ix2])+par.ls.gamma/(Lmax*par.L)).*exp(LL([ix1,ix2]));
...
% Get expectations
mu  = reshape(sum(V.*c_vec,2),par.D,par.L*par.ls.ncop); 
...
for d=1:par.D
    ...
    for l1=1:par.L
        for l2 = 1:Lmax
            ...
            % Associability
            tmp = par.ls.alpha0*lsb(l)*(B(:,:,d,l)\c_vec(d,:,l)')';
